...
use mf_cert::{decode_locus_packet_report, encode_locus_packet_for_report};
use mf_core::{
...
use mf_qc0::{parse_qc0, render_qc0};
use mf_qk0::{document_to_qk0, expand_qk0, qk0_to_document};
use mf_qm0::{parse_qm0, render_qm0};
...
pub fn report_to_document_with_registry(
        ...
        receipts: report
            ...
            .chain(
                report
                    .adequacy_records
                    .iter()
                    .filter(|item| item.verdict == mf_core::CertificationVerdict::Certified)
    ...
    if let Some(envelope) = &report.execution_envelope {
        ...
        if let (Some(lock_id), Some(manifest_id)) = (&envelope.lock_id, &envelope.manifest_id) {
            ...
            entries.push(QaEntry::LockReceipt(mf_core::LockReceipt {
    ...
    entries.extend(
        report
            ...
            .enumerate()
            .map(|(index, item)| {
                QaEntry::AtlasDeficiency(mf_core::adequacy_record_deficiency(
                    &report.theorem_id,
    ...
    entries.extend(report.obligations.iter().enumerate().map(|(index, item)| {
        QaEntry::AtlasDeficiency(mf_core::obligation_status_deficiency(
            &report.theorem_id,
            report.selected_atlas_cell.as_deref(),
    ...
    if let Some(explanation) = &report.route_explanation {
        entries.push(QaEntry::AtlasDeficiency(
            mf_core::route_explanation_deficiency(
                &report.theorem_id,
                report.selected_atlas_cell.as_deref(),
...
fn promoted_operator_object(report: &CertificationReport, id: &str) -> mf_core::QcObject {
    ...
    mf_core::QcObject {
        id: id.to_string(),
        identity: mf_core::IdentityFace {
            tag: mf_core::ObjectTag::Opr,
            cid: format!("cid:{id}"),
            codebook: "QC0_CORE".into(),
            remap: "none".into(),
            lineage: format!("derived-from:{}", report.theorem_id),
        },
        structural: mf_core::StructuralFace {
            head: "operator".into(),
            args: vec![
        ...
        constraint: mf_core::ConstraintFace {
            regime: "R_CALC".into(),
        ...
        evidence: mf_core::EvidenceFace {
            ...
            maturity: mf_core::EvidenceMaturity::Certified,
            gate_verdict: mf_core::GateVerdict::Pass,
        },
        alias: mf_core::AliasFace {
            aliases: vec![alias],
...
pub fn import_text(
    ...
    let (header, document, defaults_used, alias_expansions, loss_classes) = match surface_kind {
        ...
        SurfaceKind::Qa0 => {
            ...
            let document = mf_qa0::parse_document(&lines.collect::<Vec<_>>().join("\n"))?;
            (
                header,
                document,
                Vec::new(),
...
pub fn export_document(
    ...
    let rendered = match target {
        ...
        SurfaceKind::Qa0 => format!(
            ...
            mf_qa0::normalize_document(document)?
        ),
