...
use mf_command::{BundlePolicyArg, OptimizerPolicyArg, SurfaceArg};
use mf_core::{
...
use mf_locus::{compile_rna_to_dna_packet, sequence_dna_to_rna};
use mf_qa0::{normalize_document as normalize_qa0_document, parse_document as parse_qa0_document};
...
use mf_runtime::{RuntimeWorld, exec_host};
use mf_selector::{AtlasSelector, CampaignCertifier};
use mf_surfaces::{
...
fn main() -> Result<()> {
    std::thread::Builder::new()
        .name("mf-cli-main".into())
        .stack_size(32 * 1024 * 1024)
        .spawn(real_main)
        .map_err(|err| anyhow!("failed to start mf-cli main thread: {err}"))?
        .join()
        .map_err(|_| anyhow!("mf-cli main thread panicked"))?
}
    ...
    match cli.command {
            ...
            if let Some(surface) = report_surface {
                for value in &result {
                    if let Some(report) = value.get("theorem_id").and_then(|_| {
                        serde_json::from_value::<mf_core::CertificationReport>(value.clone()).ok()
                    }) {
        ...
        Command::RunTheorem {
            ...
            if let Some(surface) = report_surface {
                ...
                let theorem_report = mf_core::CertificationReport {
                    ...
                    execution_envelope: Some(mf_core::DeterministicExecutionEnvelope {
        ...
        Command::NormalizeRna { file } => {
            ...
            let (normalized, receipt) =
                mf_core::normalize_rna(&input).map_err(anyhow::Error::msg)?;
            println!(
                "{}",
                serde_json::to_string_pretty(&serde_json::json!({
        ...
        Command::ResearchDeriveFromReport { report_id, persist } => {
            ...
            let benchmark_schemas = mf_research::derive_benchmark_schemas_from_report(&report);
            let coverage = dispatch_proof_coverage(&report);
        ...
        Command::ResearchGovernReport { report_id, persist } => {
            ...
            let (task, signature, route_assignment, readiness, handoff) =
                mf_research::govern_report_from_cold_result(&report)?;
            if persist {
        ...
        Command::ResearchSeedExportRemediation { persist } => {
            let entries = if persist {
                ...
                mf_research::seed_export_remediation_entries()
            };
            println!(
                "{}",
                serde_json::to_string_pretty(&serde_json::json!({
...
fn normalize_file(
...
) -> Result<(String, mf_core::FormatTransformReceipt)> {
    let source_kind = infer_surface_kind(file, as_kind)?;
    ...
    if source_kind == SurfaceKind::Qa0 && !input.trim_start().starts_with("!qa0 ") {
        ...
        let receipt = mf_core::FormatTransformReceipt {
            ...
            object_ids: document
                ...
                .collect(),
            transform_kind: mf_core::TransformKind::Normalize,
            policy_id: policy.id,
            ...
            verdict: mf_core::TransformVerdict::Lossless,
            rollback_ref: None,
...
fn load_document(
    ...
    as_kind: Option<SurfaceArg>,
    registry: &SeedRegistry,
) -> Result<(QaDocument, mf_core::FormatTransformReceipt)> {
    let source_kind = infer_surface_kind(file, as_kind)?;
    let input = fs::read_to_string(file).with_context(|| format!("failed to read `{file}`"))?;
    if source_kind == SurfaceKind::Qa0 && !input.trim_start().starts_with("!qa0 ") {
        let document = parse_qa0_document(&input)?;
        ...
        let receipt = mf_core::FormatTransformReceipt {
            ...
            object_ids: document
                ...
                .collect(),
            transform_kind: mf_core::TransformKind::Import,
            policy_id: policy.id,
            ...
            verdict: mf_core::TransformVerdict::ReceiptedLoss,
            rollback_ref: None,
            replay_ref: Some("legacy-qa0".into()),
        };
...
fn certify_with_registry(
...
) -> Result<mf_core::CertificationReport> {
    let atlas = CompiledAtlas::compile(registry).map_err(anyhow::Error::msg)?;
    ...
    let report = match (
        ...
        _ => Err(mf_cert::CertError::Message(
...
fn certify_bundle_world(
    ...
    overlay_only: bool,
    options: &CertificationOptions,
) -> Result<Vec<mf_core::CertificationReport>> {
    if overlay_only {
...
fn export_report_sidecar(
    report: &mf_core::CertificationReport,
    surface: SurfaceKind,
...
fn load_report_document_for_export(
    ...
    if let Ok(report) = serde_json::from_str::<mf_core::CertificationReport>(&text) {
        if let Some(bundle_id) = report
            .execution_envelope
            .as_ref()
            .and_then(|item| item.bundle_id.clone())
