...
pub fn encode_section_packet<T: Serialize>(
    ...
    let packet = LocusPacket {
        header: LocusPacketHeader {
            ...
            integrity_hash: format!("{:x}", mf_core::stable_hash_u64(schema_hash)),
...
pub fn compile_rna_to_dna_packet(
    ...
    let lowering_receipt = SemanticLoweringReceipt {
        id: format!("SLR_{:x}", mf_core::stable_hash_u64(subject_id)),
    ...
    let packet = LocusPacket {
        header: LocusPacketHeader {
            ...
            integrity_hash: format!(
                "{:x}",
                mf_core::stable_hash_u64(&canonical_graph.canonical_text)
            ),
            strand_manifest,
            feature_flags: 0,
            root_subject_id: subject_id.into(),
...
pub fn sequence_dna_to_rna(bytes: &[u8]) -> Result<RnaCompilationArtifact, LocusIoError> {
    ...
    let normalized = NormalizedRna {
        ...
        state: mf_core::RnaState::Stabilized,
    ...
    let ssr_receipt = SsrReceipt {
        id: format!(
            "SSR_RCP_{:x}",
            mf_core::stable_hash_u64(&canonical_graph.root_id)
        ),
        ...
        transition_table_hash: mf_core::hash_serialized(&mf_core::ssr_transition_specs()),
        ephemeral: true,
    ...
    let cnorm_receipt = CnormReceipt {
        ...
        rule_table_hash: mf_core::hash_serialized(&mf_core::canonical_rule_specs()),
        idempotent: true,
    ...
    let lowering_receipt = SemanticLoweringReceipt {
        id: format!(
            "SLR_{:x}",
            mf_core::stable_hash_u64(&packet.header.root_subject_id)
        ),
