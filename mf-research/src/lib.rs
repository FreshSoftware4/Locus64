...
use mf_locus::{read_section_packet_or_json, write_section_packet};
...
fn score_routes_internal(
    ...
    let unsupported_coverage = coverage_dispatches
        .iter()
        .filter(|item| {
            matches!(item.decision, mf_core::CoverageDecision::Unsupported)
                && any_subject_match(&item.subject_id, subject_keys.iter())
        })
        .count() as f64;
        ...
        match route {
            ResearchRouteClass::Derive => {
                ...
                if matches!(
                    ...
                    mf_core::DerivationStructuralType::Projection
                        | mf_core::DerivationStructuralType::Closure
                        | mf_core::DerivationStructuralType::Reduction
                ) {
                    fit += 3.0;
            ...
            ResearchRouteClass::Benchmark => {
                ...
                if matches!(
                    signature.structural_type,
                    mf_core::DerivationStructuralType::BenchmarkFit
                ) {
                    fit += 3.0;
                }
            ...
            ResearchRouteClass::ChallengeResponse => {
                ...
                if matches!(
                    signature.structural_type,
                    mf_core::DerivationStructuralType::ChallengeResponse
                        | mf_core::DerivationStructuralType::OperationalHardening
                ) {
                    fit += 3.0;
                }
                leverage += 2.0;
            ...
            ResearchRouteClass::OperationalHardening => {
                if matches!(
                    signature.structural_type,
                    mf_core::DerivationStructuralType::OperationalHardening
                        | mf_core::DerivationStructuralType::RegistryHardening
                ) {
                    fit += 5.0;
                }
                leverage += 2.5;
...
pub fn derive_task_envelope_from_report(report: &CertificationReport) -> TaskEnvelope {
    let objective_kind = if !report.deficiencies.is_empty() {
        ...
        || matches!(
            report.verdict,
            mf_core::CertificationVerdict::Integrated | mf_core::CertificationVerdict::Certified
        )
    {
    ...
    TaskEnvelope {
        ...
        {
            mf_core::ResearchSector::Crosscut
        } else {
            mf_core::ResearchSector::Control
        },
        objective_kind,
        desired_output,
...
pub fn derive_derivation_signature_from_report(
    ...
    let risk_class = if !report.deficiencies.is_empty() {
        mf_core::RiskClass::High
    } else {
        mf_core::RiskClass::Medium
    };
    DerivationSignature {
...
pub fn derive_framework_registry_entry_from_report(
    ...
    let status = match report.verdict {
        mf_core::CertificationVerdict::Integrated => RegistryEntryStatus::Integrated,
        mf_core::CertificationVerdict::Certified => RegistryEntryStatus::Certified,
        mf_core::CertificationVerdict::BlockedOpen => RegistryEntryStatus::Blocked,
...
pub fn derive_review_receipt_from_report(report: &CertificationReport) -> ReviewReceipt {
    let verdict = match report.verdict {
        ...
        mf_core::CertificationVerdict::BlockedOpen => ReviewDecision::Hold,
    ...
    let (fit, risk, capture, continuity) = match report.verdict {
        ...
        mf_core::CertificationVerdict::BlockedOpen => (
            ReviewStatus::Revise,
            ReviewStatus::Revise,
            ReviewStatus::Pass,
            ReviewStatus::Revise,
...
pub fn derive_challenge_record_from_report(
    ...
    let grounds = if !report.deficiencies.is_empty() {
        ...
        report.verdict,
        mf_core::CertificationVerdict::BlockedOpen
            | mf_core::CertificationVerdict::BlockedContradiction
    ) {
    ...
    let severity = match report.verdict {
        mf_core::CertificationVerdict::BlockedContradiction => ChallengeSeverity::Blocking,
        mf_core::CertificationVerdict::BlockedOpen => ChallengeSeverity::High,
...
pub fn seed_export_remediation_entries() -> Vec<RemediationLedgerEntry> {
    vec![
        RemediationLedgerEntry {
            ...
            affected_surfaces: vec!["mf-admin explain-execution".into(), "execution cache".into(), "observation/report/manifest/lock resolution".into()],
        ...
        RemediationLedgerEntry {
            ...
            affected_surfaces: vec!["mf-cli export-report".into(), "mf-cli export-validation-bundle".into(), "mf-cli validate".into()],
        ...
        RemediationLedgerEntry {
            ...
            affected_surfaces: vec!["mf-admin predict-impact".into(), "command contract surfaces".into()],
        ...
        RemediationLedgerEntry {
            ...
            affected_surfaces: vec!["mf-research".into(), "routing".into(), "challenge workflow".into(), "registry".into()],
...
pub fn derive_lineage_record_from_report(report: &CertificationReport) -> ResearchLineageRecord {
    ...
    let canonical_hash = report
        ...
        .unwrap_or_else(|| {
            format!(
                "{:x}",
                mf_core::stable_hash_u64(&format!(
                    "{}|{}|{:?}",
    ...
    let failure_records = invariant_checks
        ...
        .map(|check| mf_core::FailureRecord {
    ...
    let validation_result = if failure_records.is_empty() {
        mf_core::PhaseValidationResult::Passed
    } else {
        mf_core::PhaseValidationResult::Failed
    };
    let promotion_signal = if failure_records.is_empty() {
        mf_core::PromotionSignal::Promote
    } else {
        mf_core::PromotionSignal::Hold
    };

    ResearchLineageRecord {
        id: format!("LIN_{}", subject_id),
        subject_id: subject_id.clone(),
        artifact_class: mf_core::GenomeArtifactClass::Gene,
        source_surface: mf_core::GenomeSurface::Dna,
        target_surface: mf_core::GenomeSurface::Dna,
        grammar_id: "report-envelope.v1".into(),
        canonical_hash: canonical_hash.clone(),
        lowering_receipt_id,
        phase_ids: vec![
            mf_core::PhaseId::DnaProtocolFreeze,
            mf_core::PhaseId::ResearchHostReconnect,
        ],
        phase_ledger: vec![mf_core::ChangeLedgerEntry {
            phase_id: mf_core::PhaseId::ResearchHostReconnect,
            input_state_hash: canonical_hash.clone(),
            output_state_hash: Some(format!("{:x}", mf_core::stable_hash_u64(&subject_id))),
            dependency_edges,
            invariant_checks,
            failure_records,
            validation_result,
...
pub fn derive_math_claim_packet_from_report(report: &CertificationReport) -> MathClaimPacket {
    let sector = if report.theorem_id.contains("CHAIN") || report.theorem_id.contains("BAYES") {
        ...
        mf_core::ResearchSector::Thermo
    } else {
        mf_core::ResearchSector::Control
    };
    let truth_class = if matches!(report.verdict, mf_core::CertificationVerdict::Integrated) {
        AuthorityState::Evidence
    } else if matches!(report.verdict, mf_core::CertificationVerdict::Certified) {
    ...
    ReductionMapRecord {
        ...
        residual_obligation_ids: report
            ...
            .filter(|s| {
                !matches!(
                    s.verdict,
                    mf_core::CertificationVerdict::Integrated
                        | mf_core::CertificationVerdict::Certified
                ) && !matches!(
                    s.evaluation_mode,
                    mf_core::ObligationEvaluationMode::RecomputedExact
                )
            })
...
pub fn derive_benchmark_run_record_from_report(
    ...
    BenchmarkRunRecord {
        ...
        status: match report.verdict {
            mf_core::CertificationVerdict::Integrated => RegistryEntryStatus::Integrated,
            mf_core::CertificationVerdict::Certified => RegistryEntryStatus::Certified,
            mf_core::CertificationVerdict::Benchmarked => RegistryEntryStatus::Benchmarked,
            mf_core::CertificationVerdict::BlockedOpen
            | mf_core::CertificationVerdict::BlockedContradiction => RegistryEntryStatus::Blocked,
            _ => RegistryEntryStatus::Derived,
        },
        summary: format!(
...
pub fn derive_governed_promotion_readiness_from_report(
    ...
    if let Some(coverage) = coverage {
        ...
        if matches!(coverage.decision, mf_core::CoverageDecision::Unsupported) {
    ...
    if let Some(lineage) = lineage_record {
        ...
        if lineage
            .phase_ledger
            .iter()
            .any(|entry| entry.validation_result != mf_core::PhaseValidationResult::Passed)
        {
            readiness
                .blockers
