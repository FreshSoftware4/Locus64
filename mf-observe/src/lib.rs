...
use mf_bundle::{BundleWorld, load_bundle_world};
use mf_cert::{
...
fn receipt_count(receipts: &[mf_core::ObligationEvidenceReceipt]) -> usize {
    receipts
        .iter()
...
struct ExecutedTaskBatch {
    ...
    lane_records: Vec<mf_core::LaneExecutionRecord>,
    schedule_hash: ExecutionScheduleHash,
    coherence_receipts: Vec<mf_core::ConcurrencyCoherenceReceipt>,
...
pub fn observe_report(
    ...
    if let Some(envelope) = &report.execution_envelope {
        ...
        cache_events.push(mf_core::CacheReplayEvent {
        ...
        "Evaluator" => (
            if baseline_report.obligations.iter().any(|item| {
                matches!(
                    ...
                    mf_core::ObligationEvaluationMode::Unsupported
                        | mf_core::ObligationEvaluationMode::RecomputedApproximate
                        | mf_core::ObligationEvaluationMode::RecomputedPartial
                )
            }) {
...
pub fn execute_plan(
    ...
    let execution_scope = ExecutionScope {
        ...
        cache_namespace: mf_core::CacheNamespace {
            id: root_resolution
                .cache_root
                .namespace
                .clone()
...
fn classify_evaluator_prediction(
    ...
    if resolution.evaluator.unsupported_mode == mf_core::UnsupportedHandlingMode::StrictFail
...
fn diff_dependencies(
    left: &[mf_core::BundleDependency],
    right: &[mf_core::BundleDependency],
) -> Vec<String> {
    let left_set = left
        .iter()
        .map(|item| item.id.clone())
...
fn scheduler_policy_for(
    ...
    let mut scheduler_config = prediction
        ...
        .unwrap_or(mf_core::SchedulerPolicyConfig {
        ...
        .map(|resolution| {
            (
                ...
                matches!(
                    ...
                    mf_core::UnsupportedHandlingMode::StrictFail
                ),
                match resolution.replay_cache.trust_class {
                    mf_core::ReplayTrustClass::ExactPolicyOnly => ReuseTrustClass::ExactOnly,
                    mf_core::ReplayTrustClass::AllowSurfaceOnlyChanges => {
                        ReuseTrustClass::SurfaceStable
                    }
                    mf_core::ReplayTrustClass::AllowApproximateReuse => {
...
fn build_execution_manifest_for_prediction(
    ...
    lane_records: &[mf_core::LaneExecutionRecord],
    schedule_hash: &ExecutionScheduleHash,
    ordering_receipt: &OrderingReceipt,
    coherence_receipts: &[mf_core::ConcurrencyCoherenceReceipt],
    obligation_plans: &[mf_core::ObligationPlan],
    obligation_lanes: &[mf_core::ObligationLaneRecord],
    obligation_ordering_receipts: &[mf_core::ObligationOrderingReceipt],
    obligation_merge_receipts: &[mf_core::ObligationMergeReceipt],
    replay_legality_checks: &[mf_core::ReplayLegalityCheck],
    replay_barrier_receipts: &[mf_core::ReplayBarrierReceipt],
    replay_merge_receipts: &[mf_core::ReplayMergeReceipt],
    replay_divergence_records: &[mf_core::ReplayDivergenceRecord],
    obligation_cache_shards: &[mf_core::ObligationCacheShard],
    obligation_write_sets: &[mf_core::ObligationWriteSet],
    obligation_collision_reports: &[mf_core::ObligationCollisionReport],
    obligation_namespace_receipts: &[mf_core::ObligationNamespaceReceipt],
        ...
        PredictionTarget::ProposedBundle {
            ...
            resolve_policy_graph(
                ...
                reports
                    ...
                    .unwrap_or(mf_core::OptimizerPolicy::Conservative),
            )
            .map_err(anyhow::Error::msg)?
            .resolution,
        ),
    ...
    let mut manifest = mf_policy::build_execution_manifest(
        &world.manifest.id,
        &bundle_hash,
...
fn run_prediction_target(
    ...
    let coherence = mf_core::ConcurrencyCoherenceReceipt {
...
fn assign_lanes(tasks: &[ExecutionTask], worker_count: usize) -> Vec<mf_core::LaneExecutionRecord> {
    let mut lanes = (0..worker_count)
        .map(|index| mf_core::LaneExecutionRecord {
        ...
        CertificationReport {
            ...
            policy_resolution: Some(PolicyResolution {
                ...
                optimizer: OptimizerPolicyConfig {
                    ...
                    active_axes: vec![mf_core::OptimizationAxis::Lawfulness],
                ...
                report: ReportPolicyConfig {
                    export_surfaces: vec![mf_core::SurfaceKind::Qc0],
                    include_policy_trace: true,
                    include_route_explanation: true,
                    include_obligation_logs: true,
                },
                scheduler: mf_core::SchedulerPolicyConfig {
                    parallelization: mf_core::ParallelizationPolicy::Serialize,
                    max_workers: 1,
                    allow_parallel_replay: false,
                    allow_parallel_certification: true,
                    allow_parallel_exports: true,
            ...
            route_explanation: Some(RouteExplanation {
                ...
                axes_used: vec![mf_core::OptimizationAxis::Lawfulness],
