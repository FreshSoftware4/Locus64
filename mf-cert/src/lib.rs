...
use mf_kernel::ConstitutionKernel;
use mf_locus::{decode_section_payload, decode_summary as decode_locus_summary};
use mf_policy::resolve_policy_graph;
use mf_runtime::{
...
const EVALUATOR_VERSION: &str = "mf-cert-obl-v8";
...
pub fn certify_derived_theorem_with_options(
        ...
        .select_policy_driven(
            ...
            Some(&mf_core::Budget {
                ...
                allow_lossy_supported: target
                    ...
                    .contains(&mf_core::ReversibilityClass::LossySupported),
                require_proof: true,
            }),
        ...
        )
    {
        verdict = if deficiencies
            .iter()
            .any(|item| item.class == mf_core::AtlasDeficiencyClass::DOpenConjectural)
        {
            CertificationVerdict::Underspecified
        } else {
...
fn proof_family_ok(
    ...
    match family {
        ...
        RequiredProofShapeFamily::Triangle => proof_shapes.iter().any(|id| {
            registry
                .get_proof_shape(id)
                .map(|shape| shape.kind == mf_core::ProofShapeKind::Triangle)
                .unwrap_or(false)
        }),
        RequiredProofShapeFamily::Square => proof_shapes.iter().any(|id| {
            registry
                .get_proof_shape(id)
                .map(|shape| shape.kind == mf_core::ProofShapeKind::Square)
                .unwrap_or(false)
        }),
        RequiredProofShapeFamily::Diamond => proof_shapes.iter().any(|id| {
            registry
                .get_proof_shape(id)
                .map(|shape| shape.kind == mf_core::ProofShapeKind::Diamond)
                .unwrap_or(false)
        }),
        RequiredProofShapeFamily::Pentagon => proof_shapes.iter().any(|id| {
            registry
                .get_proof_shape(id)
                .map(|shape| shape.kind == mf_core::ProofShapeKind::Pentagon)
                .unwrap_or(false)
        }),
        RequiredProofShapeFamily::Hexagon => proof_shapes.iter().any(|id| {
            registry
                .get_proof_shape(id)
                .map(|shape| shape.kind == mf_core::ProofShapeKind::Hexagon)
                .unwrap_or(false)
        }),
    }
}
...
fn collect_campaign_deficiencies(
    ...
    adequacy_records: &[mf_core::AdequacyRecord],
    proof_family_ok: bool,
) -> Vec<mf_core::AtlasDeficiency> {
    ...
    if !proof_family_ok {
        deficiencies.push(mf_core::AtlasDeficiency {
            id: format!("DGN_{}_PROOF_FAMILY", theorem_id),
            class: mf_core::AtlasDeficiencyClass::DNoCommutingProof,
            ...
            blocking_scope: mf_core::blocking_scope(true),
            control_effects: mf_core::blocking_control_effects(true),
            suggested_seam: Some(
    ...
    for obligation in obligations {
        ...
        {
            deficiencies.push(mf_core::obligation_status_deficiency(
                theorem_id,
                atlas_cell_id,
                format!(
                    "DGN_OBL_{}_{}_{}",
    ...
    if theorem_scoped_clauses.is_empty()
        || (!supports_active_adequacy(theorem_id)
            && theorem_scoped_clauses.iter().all(|clause| {
                !matches!(
                    clause.kind,
                    mf_core::AdequacyClauseKind::ProjectionInterpretation
                        | mf_core::AdequacyClauseKind::ContainmentInterpretation
                        | mf_core::AdequacyClauseKind::ClosureInterpretation
                        | mf_core::AdequacyClauseKind::RunningLawInterpretation
...
type GenericAdequacyEvaluator = fn(
    ...
    &mf_core::AdequacyClause,
) -> (CertificationVerdict, bool, String, Vec<String>);

fn generic_adequacy_evaluator(kind: &mf_core::AdequacyClauseKind) -> GenericAdequacyEvaluator {
    match kind {
        mf_core::AdequacyClauseKind::EvidenceContractInterpretation => {
            eval_evidence_contract_adequacy
        }
        mf_core::AdequacyClauseKind::BenchmarkInterpretation => eval_benchmark_adequacy,
        mf_core::AdequacyClauseKind::StressInterpretation => eval_stress_adequacy,
        mf_core::AdequacyClauseKind::ChallengeInterpretation => eval_challenge_adequacy,
        mf_core::AdequacyClauseKind::ProjectionInterpretation
        | mf_core::AdequacyClauseKind::ContainmentInterpretation
        | mf_core::AdequacyClauseKind::ClosureInterpretation
        | mf_core::AdequacyClauseKind::RunningLawInterpretation => eval_missing_host_pack_adequacy,
...
fn eval_stress_adequacy(
    ...
    let receipts = clause
        ...
        .iter()
        .filter_map(|id| registry.get_benchmark_receipt(id))
        .filter(|item| item.role == mf_core::BenchmarkRole::Stress)
        .collect::<Vec<_>>();
...
fn eval_challenge_adequacy(
    ...
    let ok = !receipts.is_empty()
        && receipts
            .iter()
            .all(|item| item.status != mf_core::ChallengeStatus::Open);
    (
        if ok {
            CertificationVerdict::Certified
        } else {
...
fn eval_underspecified_adequacy(
    _registry: &(dyn RegistryLookup + Sync),
    _clause: &mf_core::AdequacyClause,
) -> (CertificationVerdict, bool, String, Vec<String>) {
    (
        CertificationVerdict::Underspecified,
        false,
...
fn adequacy_record_deficiency(
    theorem_id: &str,
    atlas_cell_id: Option<&str>,
    record: &mf_core::AdequacyRecord,
) -> mf_core::AtlasDeficiency {
    mf_core::adequacy_record_deficiency(
        theorem_id,
        atlas_cell_id,
...
fn deficiency_blocks_campaign(deficiencies: &[mf_core::AtlasDeficiency]) -> bool {
    deficiencies.iter().any(|item| {
        item.control_effects
            .iter()
            .any(|effect| effect == &mf_core::DeficiencyControlEffect::BlockCampaign)
    })
}

fn deficiency_blocks_promotion(deficiencies: &[mf_core::AtlasDeficiency]) -> bool {
    deficiencies.iter().any(|item| {
        item.control_effects
            .iter()
            .any(|effect| effect == &mf_core::DeficiencyControlEffect::BlockPromotion)
    })
}

fn collect_receipt_deficiencies(
    deficiencies: &mut Vec<mf_core::AtlasDeficiency>,
    theorem_id: &str,
    atlas_cell_id: Option<&str>,
    obligation_id: &str,
    ...
    if receipt.subreceipts.is_empty() {
        if receipt.verdict != CertificationVerdict::Certified {
            deficiencies.push(mf_core::receipt_deficiency(
                theorem_id,
                atlas_cell_id,
                format!("DGN_EQR_{}_{}", obligation_id, receipt.id),
                receipt,
                mf_core::AtlasDeficiencyClass::DNoAdequacy,
                mf_core::AtlasDeficiencyClass::DOpenConjectural,
                format!(
...
fn collect_promotion_artifact_ids(
    ...
    obligations: &[ObligationStatus],
    adequacy_records: &[mf_core::AdequacyRecord],
    deficiencies: &[mf_core::AtlasDeficiency],
    verdict: &CertificationVerdict,
) -> Vec<String> {
    if !is_chain_rule_family(&theorem.id)
        || target.promotion_goal != mf_core::PromotionGoal::PromoteOperator
        || *verdict != CertificationVerdict::Certified
        || adequacy_records
            .iter()
        ...
        if let Some(shape) = registry.get_proof_shape(proof_shape_id) {
            receipts.push(checker_receipt(
                ...
                kernel
                    ...
                    .map_err(mf_kernel::KernelError::Message)
...
fn checker_receipt(
    ...
    subject_id: &str,
    result: Result<String, mf_kernel::KernelError>,
) -> CheckerReceipt {
    let id = format!(
        "CHK_{}",
...
fn chain_rule_promoted_operator(
    ...
    if object.constraint.regime == "R_CALC"
        && matches!(
            ...
            mf_core::EvidenceMaturity::Validated | mf_core::EvidenceMaturity::Certified
        )
        && object.evidence.gate_verdict == mf_core::GateVerdict::Pass
    {
...
fn evaluate_obligation(
    ...
    evaluator_policy: &mf_core::EvaluatorPolicyConfig,
) -> ObligationStatus {
...
fn evaluate_obligations(
    ...
    evaluator: &(impl ObligationEvaluator + Sync),
    policy_resolution: &mf_core::PolicyResolution,
    options: &CertificationOptions,
...
fn cache_entry_valid(
    ...
    resolution: Option<&mf_core::PolicyResolution>,
) -> bool {
...
pub fn encode_locus_packet_for_report(report: &CertificationReport) -> Result<Vec<u8>, CertError> {
    ...
    mf_core::encode_locus_packet(&packet).map_err(CertError::Message)
}
...
fn decode_cached_report_packet(bytes: &[u8]) -> Result<CachedCertificationReport, CertError> {
    let packet = mf_core::decode_locus_packet(bytes).map_err(CertError::Message)?;
    let header = packet
        .sections
        .iter()
...
mod tests {
    use super::*;
    use mf_bundle::import_bundle_file;
...
campaign {"id":"CPG_T","theorem":"THS_T","target_profile":"TGT_T","route_ledger":"TRL_T","obligations":["OBL_T"],"certificates":[],"dependencies":[],"campaign_class":"CBasic","verdict":"Benchmarked","payoff":["test"]}"#,
        ...
        .unwrap();
        let world =
            import_bundle_file(&file, None, mf_core::BundleConflictPolicy::Reject, None).unwrap();
    ...
    fn imported_multi_campaign_bundle_serializes_reports() {
        ...
        let dir = std::env::temp_dir().join("mf_cert_bundle_test_multi");
        let _ = fs::create_dir_all(&dir);
        let file = dir.join("bundle.qc0");
        fs::write(
            &file,
...
adequacy {"id":"ADQ_G2_STRESS","kind":"StressInterpretation","regime_ids":["R_TOP","R_CALC"],"bridge_ids":[],"theorem_ids":["THS_G2"],"burden_pack_ids":["BPK_G2"],"claim_packet_ids":["CLM_G2"],"evidence_contract_ids":["ECT_G2"],"benchmark_receipt_ids":["BMR_G2_TARGET"],"challenge_receipt_ids":[],"reproducibility_packet_ids":["RPK_G2"],"description":"stress coverage required","blocking":true}"#,
        ...
        assert!(
            report
                .deficiencies
                .iter()
                .any(|item| item.class == mf_core::AtlasDeficiencyClass::DStressGap)
        );
    }
}
