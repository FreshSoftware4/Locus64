...
impl ConstitutionKernel {
    ...
    pub fn validate_regime(&self, regime: &l64_core::RegimePack) -> Result<(), KernelError> {
        let required = [
    ...
    pub fn validate_bridge(
        &self,
        bridge: &l64_core::BridgeContract,
        registry: &dyn RegistryLookup,
    ) -> Result<(), KernelError> {
        ...
        if bridge.reversibility == l64_core::ReversibilityClass::Invalid {
            return Err(KernelError::Message(format!(
    ...
    pub fn validate_certificate(
        &self,
        certificate: &l64_core::Certificate,
        registry: &dyn RegistryLookup,
    ) -> Result<(), KernelError> {
    ...
    pub fn validate_adequacy_clause(
        ...
        if matches!(clause.kind, l64_core::AdequacyClauseKind::BridgeSoundness)
    ...
    pub fn validate_obligation_status(&self, status: &ObligationStatus) -> Result<(), KernelError> {
        ...
        match status.evaluation_mode {
            ObligationEvaluationMode::RecomputedExact => {
                if status.verdict != l64_core::CertificationVerdict::Certified {
                    return Err(KernelError::Message(format!(
...
fn receipt_has_residual(receipt: &l64_core::ObligationEvidenceReceipt) -> bool {
    if receipt.subreceipts.is_empty() {
        return !receipt.computed || receipt.verdict != l64_core::CertificationVerdict::Certified;
    }
...
impl ComposeBridge for ConstitutionKernel {
    fn compose_bridge_path(
        &self,
        bridges: &[l64_core::BridgeContract],
        ...
        if let Some(budget) = budget {
            ...
            if !budget.allow_lossy_supported
                && bridges.iter().any(|bridge| {
                    bridge.reversibility == l64_core::ReversibilityClass::LossySupported
                })
            {
                return Err(
