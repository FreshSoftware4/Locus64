...
fn temp_dir() -> PathBuf {
    let root = std::env::temp_dir().join("l64_admin_tests").join(
        SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos()
...
fn clear_cache() {
    let output = Command::new(cargo_bin("l64-cli"))
        .current_dir(workspace_root())
        .env("MF_CACHE_NAMESPACE", test_namespace())
...
fn lock_manifest_and_replay_flow_work() {
    ...
    let manifest = run_json("l64-admin", &["dump-execution-manifest", &manifest_id]);
    ...
    let diff = run_json("l64-admin", &["compare-locks", &lock_id, &other_lock_id]);
    assert!(
...
fn observe_predict_plan_and_assess_route_drift_workflow() {
    ...
    let observed = run_json(
        "l64-admin",
        &["observe-run", "--report", baseline_report_id],
    );
    assert_eq!(
...
fn execute_plan_reconciles_predicted_route_drift() {
    ...
    let execution = run_json(
        "l64-admin",
        &["execute-plan", "--prediction", prediction_id],
    );
    assert!(
        execution["outcomes"]
            .as_array()
...
fn runtime_roots_and_artifact_resolution_use_workspace_root() {
    ...
    let roots = run_json("l64-admin", &["dump-runtime-roots"]);
    assert!(
    ...
    let locator = run_json("l64-admin", &["resolve-artifact-path", &manifest_id]);
    assert!(
        locator["normalized_path"]
            .as_str()
            .unwrap()
            .contains(".l64-cache/namespaces/")
    );
    assert!(
        locator["normalized_path"]
            .as_str()
...
fn schedule_and_namespace_tools_work_without_global_serialization() {
    ...
    let namespaces = run_json_ns(&namespace, "l64-admin", &["dump-cache-namespaces"]);
    assert!(
        namespaces
            .as_array()
            .unwrap()
