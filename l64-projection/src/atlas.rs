use crate::{
    ContextAnalysis, EvidenceRef, PortRef, ProjectionError, ProjectionSource, SourceRef,
    closure_rank, evidence_ref, source_ref,
};
...
impl AtlasView {
    ...
    pub(crate) fn derive_analyzed(
        ...
        for &node in analysis.nodes() {
            ...
            let evidence = evidence_ref(graph, node, |id| analysis.closure(id))?;
            candidates.push(AtlasCandidate {
                kind,
                source,
                opcode: record.opcode(),
                closure: analysis.closure(node)?,
                output_type,
                ports,
                direct_dependents,
                canonical_route,
