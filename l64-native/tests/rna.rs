use l64_native::{
    ConstraintKind, Dimension, Obstruction, RnaError, compile_rna, dna_to_rna, normalize_rna,
    rna_to_dna,
};
...
fn rna_preserves_unknown_guard_and_refuses_refuted_guard() {
    ...
    assert_eq!(
        rna_to_dna(refuted),
        Err(RnaError::Graph(Obstruction::GuardViolated {
            subject: 3,
...
fn rna_rejects_dimensionally_invalid_addition() {
    ...
    assert_eq!(
        rna_to_dna(source),
        Err(RnaError::Graph(Obstruction::DimensionMismatch {
