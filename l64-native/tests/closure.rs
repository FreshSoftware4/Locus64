...
#[test]
fn context_closure_analysis_matches_individual_queries_exactly() {
    let fixture = guarded_fixture();
    let states = fixture.graph.context_closure_states(ROOT_CONTEXT).unwrap();
    assert_eq!(
        states.len(),
        fixture.graph.visible_nodes(ROOT_CONTEXT).unwrap().len()
    );
    for (node, state) in states {
        assert_eq!(
            state,
            fixture.graph.closure_state(ROOT_CONTEXT, node).unwrap()
        );
    }
}
