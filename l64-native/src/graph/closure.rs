impl Graph {
    ...
    pub fn context_closure_states(
        &self,
        context: ContextId,
    ) -> Result<Vec<(NodeId, ClosureState)>, Obstruction> {
        let nodes = self.visible_nodes(context)?;
        let mut memo = vec![None; self.node_count()];
        let mut states = Vec::with_capacity(nodes.len());
        for node in nodes {
            states.push((node, self.closure_state_inner(context, node, &mut memo)?));
        }
        Ok(states)
    }
    ...
    pub fn context_closure(&self, context: ContextId) -> Result<ClosureState, Obstruction> {
        ...
        for (_, node_state) in self.context_closure_states(context)? {
            state = state.combine(node_state);
