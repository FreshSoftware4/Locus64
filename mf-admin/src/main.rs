...
use mf_bundle::{BundleWorld, import_bundle_file, load_bundle_world};
use mf_cert::{
...
use mf_command::{AdminSurfaceArg as SurfaceArg, BundlePolicyArg, OptimizerPolicyArg};
use mf_core::{
...
use mf_policy::{build_execution_manifest, build_replay_lock_manifest, resolve_policy_graph};
...
fn main() -> Result<()> {
    std::thread::Builder::new()
        .name("mf-admin-main".into())
        .stack_size(32 * 1024 * 1024)
        .spawn(real_main)
        .map_err(|err| anyhow!("failed to start mf-admin main thread: {err}"))?
        .join()
        .map_err(|_| anyhow!("mf-admin main thread panicked"))?
}
...
) -> Result<mf_policy::ResolvedPolicyGraph> {
    resolve_policy_graph(
        &world.overlay,
...
fn certify_bundle_world(
    ...
    if world.overlay.local.campaigns.is_empty() {
        for theorem in &world.overlay.local.theorem_specs {
            ...
            let atlas =
                mf_atlas::CompiledAtlas::compile(&world.overlay).map_err(anyhow::Error::msg)?;
        ...
        let atlas = mf_atlas::CompiledAtlas::compile(&world.overlay).map_err(anyhow::Error::msg)?;
...
) -> Result<mf_observe::PredictionRecord> {
    let kind = match policy.kind {
        mf_core::PolicyKind::Optimizer => "Optimizer",
        mf_core::PolicyKind::Evaluator => "Evaluator",
        mf_core::PolicyKind::ReplayCache => "ReplayCache",
        _ => "Other",
    };
...
fn classify_artifact(id: &str) -> Result<serde_json::Value> {
    ...
    if let Ok(observation) = mf_observe::load_observation(id) {
        let locator = locate_known_artifact(id).ok();
...
fn resolve_execution_record(id: &str) -> Result<mf_core::PlanExecutionRecord> {
    if let Ok(record) = load_execution(id) {
        return Ok(record);
    }
    if let Ok(observation) = mf_observe::load_observation(id) {
...
fn resolve_execution_record_from_report_id(
    report_id: &str,
) -> Result<mf_core::PlanExecutionRecord> {
    let report = replay_report(report_id).map_err(anyhow::Error::msg)?;
...
fn explain_execution_fallback(id: &str) -> Result<serde_json::Value> {
    ...
    if let Ok(observation) = mf_observe::load_observation(id) {
        let route_events = observation
            ...
            .events
            .iter()
            .filter(|event| matches!(event.kind, mf_core::ExecutionEventKind::RouteSelected))
            .count();
        let obligation_traces = observation
            .record
            .events
            .iter()
            .filter(|event| matches!(event.kind, mf_core::ExecutionEventKind::ObligationEvaluated))
            .count();
        return Ok(serde_json::json!({
...
fn scan_execution_records(
    mut predicate: impl FnMut(&mf_core::PlanExecutionRecord) -> bool,
) -> Result<Option<mf_core::PlanExecutionRecord>> {
    let root = mf_core::resolve_cache_root().map_err(anyhow::Error::msg)?;
...
fn dump_cache_namespaces() -> Result<Vec<String>> {
    let project = mf_core::resolve_project_root().map_err(anyhow::Error::msg)?;
