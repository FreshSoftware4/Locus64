            ...
            .map(|campaign| {
                campaign
                    ...
                    .map(|obligation| ObligationStatus {
                        ...
                        evaluation_mode: mf_core::ObligationEvaluationMode::StoredReceiptUsed,
...
mod tests {
    ...
    #[derive(Clone)]
    struct TestRegistry {
        cells: Vec<mf_core::AtlasCell>,
        regimes: Vec<mf_core::RegimePack>,
        proof_shapes: Vec<mf_core::ProofShape>,
        bridges: Vec<mf_core::BridgeContract>,
    ...
    impl RegistryLookup for TestRegistry {
        fn get_object(&self, _id: &str) -> Option<mf_core::QcObject> {
            None
        }
        fn get_regime(&self, id: &str) -> Option<mf_core::RegimePack> {
            self.regimes.iter().find(|item| item.id == id).cloned()
        }
        fn get_bridge(&self, id: &str) -> Option<mf_core::BridgeContract> {
            self.bridges.iter().find(|item| item.id == id).cloned()
        }
        fn get_proof_shape(&self, id: &str) -> Option<mf_core::ProofShape> {
            self.proof_shapes.iter().find(|item| item.id == id).cloned()
        }
        fn get_atlas_cell(&self, _id: &str) -> Option<mf_core::AtlasCell> {
            None
        }
        fn get_mechanization_package(&self, _id: &str) -> Option<mf_core::MechanizationPackage> {
            None
        }
        fn get_theorem_spec(&self, id: &str) -> Option<mf_core::TheoremSpec> {
            self.theorem_specs
                .iter()
                .find(|item| item.id == id)
                .cloned()
        }
        fn get_obligation(&self, id: &str) -> Option<mf_core::Obligation> {
            self.obligations.iter().find(|item| item.id == id).cloned()
        }
        fn get_target_profile(&self, id: &str) -> Option<mf_core::TargetProfile> {
            self.target_profiles
                .iter()
                .find(|item| item.id == id)
                .cloned()
        }
        fn get_route_ledger(&self, id: &str) -> Option<mf_core::RouteLedger> {
            self.route_ledgers
                .iter()
                .find(|item| item.id == id)
                .cloned()
        }
        fn get_certificate(&self, id: &str) -> Option<mf_core::Certificate> {
            self.certificates.iter().find(|item| item.id == id).cloned()
        }
        fn get_campaign(&self, id: &str) -> Option<mf_core::Campaign> {
            self.campaigns.iter().find(|item| item.id == id).cloned()
        }
        fn get_campaign_portfolio(&self, _id: &str) -> Option<mf_core::CampaignPortfolio> {
            None
        }
        fn get_route_class(&self, id: &str) -> Option<mf_core::RouteClass> {
            self.route_classes
                .iter()
                .find(|item| item.id == id)
                .cloned()
        }
        fn get_atlas_deficiency(&self, _id: &str) -> Option<mf_core::AtlasDeficiency> {
            None
        }
        fn atlas_deficiencies(&self) -> Vec<mf_core::AtlasDeficiency> {
            Vec::new()
        }
        fn get_adequacy_clause(&self, _id: &str) -> Option<mf_core::AdequacyClause> {
            None
        }
        fn adequacy_clauses(&self) -> Vec<mf_core::AdequacyClause> {
            Vec::new()
        }
        fn get_codebook_pack(&self, _id: &str) -> Option<mf_core::CodebookPack> {
            None
        }
        fn get_glyph_pack(&self, _id: &str) -> Option<mf_core::GlyphPack> {
            None
        }
        fn get_combo_pack(&self, _id: &str) -> Option<mf_core::ComboPack> {
            None
        }
        fn get_projection_policy(&self, _id: &str) -> Option<mf_core::ProjectionPolicy> {
            None
        }
        fn get_alias_expansion_policy(&self, _id: &str) -> Option<mf_core::AliasExpansionPolicy> {
            None
        }
        fn get_surface_policy(&self, _id: &str) -> Option<mf_core::SurfacePolicy> {
            None
        }
        fn get_capability_matrix(&self, _id: &str) -> Option<mf_core::CapabilityMatrix> {
            None
        }
        fn get_roundtrip_report(&self, _id: &str) -> Option<mf_core::RoundTripReport> {
            None
        }
        fn get_transform_receipt(&self, _id: &str) -> Option<mf_core::FormatTransformReceipt> {
            None
        }
        fn get_surface_deficiency(&self, _id: &str) -> Option<mf_core::SurfaceDeficiency> {
            None
        }
        fn get_policy_object(&self, _id: &str) -> Option<mf_core::MechanizationPolicyObject> {
            None
        }
        fn policy_objects(&self) -> Vec<mf_core::MechanizationPolicyObject> {
            Vec::new()
        }
        fn policy_bindings(&self) -> Vec<mf_core::PolicyBinding> {
            Vec::new()
        }
        fn find_equivalence_class(
            ...
            _object_id: &str,
            _regime: &str,
        ) -> Option<mf_core::EquivalenceClass> {
            None
        }
        fn atlas_cells(&self) -> Vec<mf_core::AtlasCell> {
            self.cells.clone()
        }
    }
    ...
    fn certifier_returns_benchmarked_campaign_report() {
        let registry = TestRegistry {
            ...
            proof_shapes: vec![mf_core::ProofShape {
                id: "PS_SQUARE".into(),
                kind: mf_core::ProofShapeKind::Square,
            ...
            bridges: vec![mf_core::BridgeContract {
                ...
                enrich: vec!["der".into()],
                loss: vec![],
                reversibility: mf_core::ReversibilityClass::Enriching,
            ...
            theorem_specs: vec![TheoremSpec {
                ...
                primary_zone: mf_core::ProofMechanismZone::PmzStructural,
            ...
            target_profiles: vec![TargetProfile {
                ...
                target_equivalence: "eq".into(),
                allowed_bridge_classes: vec![mf_core::ReversibilityClass::Enriching],
                loss_ceiling: 1,
                rollback_ceiling: 1,
                required_receipt_class: "RC".into(),
                required_proof_shape_family: mf_core::RequiredProofShapeFamily::Square,
                promotion_goal: mf_core::PromotionGoal::PromoteOperator,
                primary_zone: mf_core::ProofMechanismZone::PmzStructural,
